// ======================================================================================================= //
// ===== скрипт перенесен в  другое рассширение потому что он конфликтует с панелью и ее скриптами ======= //
// ===== скрипт перенесен в C:\Projects__on__SSD_C_Win10\Twitch_Mute_ads_and_hide_them рассширение  ====== //
// ============= старая версия со спамом в логах в контейнере в которых могут отсутствовать теги ======== //
// ============= старая версия со спамом в логах в контейнере в которых могут отсутствовать теги ======== //
// ============= старая версия со спамом в логах в контейнере в которых могут отсутствовать теги ======== //
// ============= старая версия со спамом в логах в контейнере в которых могут отсутствовать теги ======== //
// ============= старая версия со спамом в логах в контейнере в которых могут отсутствовать теги ======== //
...
// ============== 16-08-2025 новая версия исправленая удалены лишние логи что исключает спам в логах ============================ //
// ============== 16-08-2025 новая версия исправленая удалены лишние логи что исключает спам в логах ============================ //
// ============== 16-08-2025 новая версия исправленая удалены лишние логи что исключает спам в логах ============================ //
// ============== 16-08-2025 новая версия исправленая удалены лишние логи что исключает спам в логах ============================ //
(function () {
    ...
    const debouncedRestoreCursorToEnd = debounce(restoreCursorToEnd, 550);
    function blurChatInput() {
        const chatInput = document.querySelector(chatInputSelector);
        if (chatInput && isChatInputFocused()) {
            // console.log('Снимаем фокус');
    ...
    document.addEventListener('blur', (event) => {
        if (event.target instanceof Element && event.target.matches(chatInputSelector)) {
            userInteracted = false;
            mouseSelecting = false;
            // console.log('>> Инпут потерял фокус — сброс флагов');
        }
   ...
   ['mouseenter', 'mouseleave'].forEach((eventName) => {
    document.addEventListener(eventName, (event) => {
        if (event.target instanceof Element && event.target.matches(tagSelector)) {
            if (isChatInputFocused() && !userInteracted && !mouseSelecting && !restorePending) {
                debouncedRestoreCursorToEnd();
            }
        }
    }, true);
});
...
const observer = new MutationObserver((mutations) => {
    if (restorePending) return; // пропускаем дубли
    
    let shouldRestore = false;
    mutations.forEach((mutation) => {
        if (mutation.addedNodes.length || mutation.removedNodes.length) {
            const popup = document.querySelector(popupSelector);
            if (popup !== null) shouldRestore = true;
        }
    });
    
    if (shouldRestore && isChatInputFocused() && !userInteracted && !mouseSelecting) {
        restorePending = true;
        setTimeout(() => {
            restoreCursorToEnd();
...
const chatContainer = document.querySelector('.chat-room') || document.body;
