...
Promise.all([
  ...
  import('./ui.js').then(module => {
      ...
      console.error("[Content] Failed to load ui:", error);
      showIsolatedNotification('Failed to load ui module', 5000);
  }),

import('./hide__twitchStreamer_Description.js').then(module => {    
    console.log('[Content] hide__twitchStreamer_Description module loaded');
    module.addToggleButton();
}).catch(error => {
    console.error("[Content] Failed to load hide__twitchStreamer_Description module:", error);
    showIsolatedNotification('Failed to load hide__twitchStreamer_Description module', 5000);
}),
...
import('./themes.js').then(module => {
 ...
 import('./emojiSpamProtection.js').then(module => {
    console.log('[Content] emojiSpamProtection module loaded');
    module.initEmojiSpamProtection();
    module.updateEmojiSpamProtectionUI();
}).catch(error => {
 ...
 import('./bindButtonHandlers.js').then(module => {
  ...
  // АвтоБан Unicode-мусора (Zalgo symbols) на твич странице стримера в описании//
        ...
        setInterval(() => {
            if (!observerIsActive) {
                ...
                const chatContainer = document.querySelector(
                    '.chat-scrollable-area__message-container, .chat-room__content, .chat-list--default, [data-a-target="chat-room-content"]'
                );
                if (chatContainer) {
...
function addUser(username) {
    ...
    try {
        ...
        const isDuplicate = bannedUsers.some(user => 
            user.text.toLowerCase() === normalizedUsername.toLowerCase() && user.type === 'user'
        );
        if (isDuplicate) {
...
function addKeyword(text, platform) {
    try {
        ...
        const isDuplicate = items.some(item => 
            item.text.toLowerCase() === text.toLowerCase() && item.type === platform
        );
        if (isDuplicate) {
...
function startKeywordFiltering() {
    ...
    const filterMessages = () => {
        ...
        messages.forEach(message => {
            const textFragment = message.querySelector('[data-a-target="chat-message-text"]');
            if (!textFragment) return;

            // Skip if already filtered
            const messageId = `${Date.now()}_${message.dataset.userId || 'unknown'}`;
            if (filteredMessages.has(messageId)) return;
...
function observeKeywordChatContainer(container = document.querySelector('.chat-scrollable-area__message-container, .chat-room__content, .chat-list--default, [data-a-target="chat-room-content"]')) {
    if (!container) {
...
function monitorKeywordChatReset() {
    ...
    const resetKeywordFiltering = () => {
        ...
        const chatContainer = document.querySelector('.chat-scrollable-area__message-container, .chat-room__content');
        if (chatContainer) {
            observeKeywordChatContainer(chatContainer);
        } else {
    ...
    const checkChatState = () => {
        ...
        const messages = chatContainer.querySelectorAll('[data-a-target="chat-line-message"], .chat-line__message');
...
function checkKeywordFilteringStatus() {
    ...
    messages.forEach((message, index) => {
        if (index > 5) return;
        const textFragment = message.querySelector('[data-a-target="chat-message-text"], .text-fragment');
        if (!textFragment) return;
            ...
            if (file) {
                ...
                reader.onload = (e) => {
                    try {
                        ...
                        const validKeywords = (data.bannedKeywords || []).filter(item =>
    ...
    try {
        ...
        input.onchange = (event) => {
            ...
            reader.onload = (e) => {
                try {
                    const data = JSON.parse(e.target.result);
                    const validUsers = (data.bannedUsers || []).filter(item =>
                        item.id && item.text && item.date
                    );
                    ...
                    if (bannedChatList) {
                        ...
                        const lastKeyword = allItems.length > 0 ? allItems.reduce((latest, current) =>
                            new Date(current.date) > new Date(latest.date) ? current : latest, allItems[0]) : null;
                    ...
                    if (isTextBlockingEnabled && window.chatFilter) {
                        ...
                        if (chatContainer) window.chatFilter.filterTextInNode(chatContainer);
                    }
                } catch (error) {
    ...
    try {
        ...
        input.onchange = (event) => {
            ...
            reader.onload = (e) => {
                try {
                    const data = JSON.parse(e.target.result);
                    const validKeywords = (data.chatBannedItems?.bannedKeywords || []).filter(item =>
                        item.id && item.text && item.date
                    );
                    const validUsers = (data.chatBannedItems?.bannedUsers || []).filter(item =>
                        item.id && item.text && item.date
                    );
        ...
        if (!isWorking || !chatContainer) {
            ...
            if (currentTime - lastFailureTime > maxFailureDuration) {
                console.log(
                    ...
                    'color:rgb(194, 86, 86); font-weight: bold;',
                    'color:rgb(204, 79, 79);'
                );
                initBlocking();
...
// ================================================================================================ //
// =================== Автоматический popup при появлении карточки смайла/эмодзи ================== //
...
function getEmotesFromChatMessage(messageContainer) {
    ...
    const emoteElements = messageContainer.querySelectorAll(
        '.chat-line__message--emote, .ffz-emote, .seventv-emote, .bttv-emote, .twitch-emote, .ffz-emoji'
    );
...
function toggleKeywordsInChat(firstUpdate = false) {
    ...
    const messages = document.querySelectorAll('[data-a-target="chat-line-message"], .chat-line__message');
...
function toggleEmotesInNode(node, immediate = false) {
    ...
    try {
        ...
        if (!node.closest('.chat-scrollable-area__message-container, .video-chat__message-list-wrapper, .ffz--inline, .chat-image chat-line__message--emote ffz--pointer-events ffz-tooltip ffz-emote, chat-line__message tw-relative')) {
...
function monitorChatReset() {
    ...
    const checkChatState = () => {
        ...
        const messages = chatContainer.querySelectorAll('.chat-line__message, .chat-message-text');
        const currentMessageCount = messages.length;
...
function getFfzTooltipEmotes() {
    ...
    if (!tooltip) return [];
    const imgs = Array.from(tooltip.querySelectorAll('img')).filter(img =>
...
function findEmoteContainerAndEmotes(clickedImg) {
    const messageContainer = clickedImg.closest(
        '.message, .chat-line__message, .chat-line__message-container, .ffz--inline, .modified-emote, .scaled-modified-emote, [class*="modified-emote"]'
    ) || clickedImg.parentElement || clickedImg;
