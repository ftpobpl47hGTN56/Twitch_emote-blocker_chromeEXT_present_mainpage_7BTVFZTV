...
let emojiSpamLimit = 6; // По умолчанию 6 эмодзи

// Функция чтения из chrome.storage.local
...
// Функция записи в chrome.storage.local
...
// Функция для проверки и скрытия сообщений с эмодзи-спамом
export function checkEmojiSpamInChat() {
    ...
    messages.forEach(message => {
        ...
        if (isEmojiSpamProtectionEnabled) {
            const emotes = messageContent.querySelectorAll(
            ...
            if (totalEmojis > emojiSpamLimit) {
                ...
                if (window.Notifications && window.Notifications.isGlobalNotificationsEnabled()) {
                    window.Notifications.showPanelNotification(
                        `Message content hidden due to emoji spam (${totalEmojis} emojis)`,
                        3000,
                        false
                    );
                }
...
// Функция для установки состояния защиты от спама эмодзи
export async function setEmojiSpamProtectionEnabled(state) {
...
// Функция для установки лимита эмодзи
export async function setEmojiSpamLimit(limit) {
    ...
    try {
        ...
        if (window.Notifications && window.Notifications.isGlobalNotificationsEnabled()) {
            window.Notifications.showPanelNotification(
                `Emoji spam limit set to ${emojiSpamLimit}`,
                3000,
                false
            );
        }
...
// Настройка MutationObserver для отслеживания новых сообщений
...
// Интеграция с существующим наблюдателем чата
export function integrateEmojiSpamProtection() {
...
// Инициализация защиты от спама эмодзи
export async function initEmojiSpamProtection() {
...
// Обновляем UI для переключателя и поля ввода лимита защиты от спама эмодзи
export async function updateEmojiSpamProtectionUI() {
...
// Добавляем в объект ChatFilter для совместимости
console.log("[EmojiSpamProtection] Initializing ChatFilter...");
window.ChatFilter = window.ChatFilter || {};
window.ChatFilter.setEmojiSpamProtectionEnabled = window.ChatFilter.setEmojiSpamProtectionEnabled || setEmojiSpamProtectionEnabled;
window.ChatFilter.setEmojiSpamLimit = window.ChatFilter.setEmojiSpamLimit || setEmojiSpamLimit;
window.ChatFilter.checkEmojiSpamInChat = window.ChatFilter.checkEmojiSpamInChat || checkEmojiSpamInChat;
window.ChatFilter.updateEmojiSpamProtectionUI = window.ChatFilter.updateEmojiSpamProtectionUI || updateEmojiSpamProtectionUI;
console.log("[EmojiSpamProtection] ChatFilter initialized:", {
    setEmojiSpamProtectionEnabled: typeof window.ChatFilter.setEmojiSpamProtectionEnabled === 'function',
    setEmojiSpamLimit: typeof window.ChatFilter.setEmojiSpamLimit === 'function',
    checkEmojiSpamInChat: typeof window.ChatFilter.checkEmojiSpamInChat === 'function',
    updateEmojiSpamProtectionUI: typeof window.ChatFilter.updateEmojiSpamProtectionUI === 'function'
});
