...
(function () {
    'use strict';
    let isLinkProcessingEnabled = false; // false; true;  // // По умолчанию обработка включена
    let observer = null;
    ...
    const emotePlatforms = {
        ...
        'imgur.com': (url) => {
            ...
            if (imageMatch)
                return `https://i.imgur.com/${imageMatch[1]}.jpg`;
            return url;
        },
        'kappalol.fun': async (url) => {
            ...
            if (videoMatch)
                return `https://kappalol.fun/${videoMatch[1]}.${videoMatch[2]}`;
            if (albumMatch)
                return `https://kappalol.fun/${albumMatch[1]}.jpg`;
            if (imageMatch)
                return `https://kappalol.fun/${imageMatch[1]}.${imageMatch[2]}`;
            if (shortIdMatch) {
        ...
        'segs.lol': async (url) => {
            ...
            if (videoMatch)
                return `https://segs.lol/${videoMatch[1]}.${videoMatch[2]}`;
            if (albumMatch)
                return `https://segs.lol/${albumMatch[1]}.jpg`;
            if (imageMatch)
                return `https://segs.lol/${imageMatch[1]}.${imageMatch[2]}`;
            if (shortIdMatch) {
        ...
        'i.nuuls.com': (url) => {
            ...
            if (albumMatch)
                return `https://i.nuuls.com/${albumMatch[1]}.jpg`;
            if (imageMatch)
                return `https://i.nuuls.com/${imageMatch[1]}.jpg`;
            return url;
        },
...
function setupMutationObserver() {
    ...
    if (isLinkProcessingEnabled) {
        observer = new MutationObserver(debouncedProcessLinks);
 ...
 // ================ этот блок удален  потому что это дубликат ====================== //
 /* function enableLinkProcessing() {
    console.log('[Content] Enabling link processing...');
    try {
        isLinkProcessingEnabled = true;
        setStorage('isLinkProcessingEnabled', true);
        console.log('[Content] isLinkProcessingEnabled set to:', isLinkProcessingEnabled);
        setupMutationObserver();
        debouncedProcessLinks(); // Запускаем обработку сразу
    } catch (error) {
        console.error('[Content] Error in enableLinkProcessing:', error);
    }
}

function disableLinkProcessing() {
    console.log('[Content] Disabling link processing...');
    try {
        isLinkProcessingEnabled = false;
        setStorage('isLinkProcessingEnabled', false);
        console.log('[Content] isLinkProcessingEnabled set to:', isLinkProcessingEnabled);
        setupMutationObserver(); // Отключаем наблюдатель
    } catch (error) {
        console.error('[Content] Error in disableLinkProcessing:', error);
    }
}

// Доступ через window
window.enableLinkProcessing = enableLinkProcessing;
window.disableLinkProcessing = disableLinkProcessing;
    ...
    if (!isLinkProcessingEnabled) {
        console.debug('Link processing is disabled');
        return;
    }
    ...
    async function replaceLinkWithMedia(link) {
        ...
        mediaElement = document.createElement('img');
            ...
            linkWrapper.dataset.processed = 'true';  // <-- Добавьте эту строку
  ...
  .chat-line__message a, [data-testid="tweet"] a {
    ...
    // ✅ start() — убрать setStorage (это обязанность ui.js), 
    //    или использовать chrome.storage напрямую:
    function start() {
            console.log('[_Previewer_Media_] Starting...');
            isLinkProcessingEnabled = true;
            setStorage('isLinkProcessingEnabled', true); // ← тот же setStorage что в ui.js
            setupMutationObserver();
            debouncedProcessLinks();
        }

        function stop() {
            console.log('[_Previewer_Media_] Stopping...');
            isLinkProcessingEnabled = false;
            setStorage('isLinkProcessingEnabled', false); // ← тот же setStorage что в ui.js
            if (observer) {
                observer.disconnect();
                observer = null;
            }
        }     
        // ✅ ЗАМЕНИТЬ на один блок в конце IIFE, перед window.PreviewerMedia = {...}:
         // Читаем через тот же getStorage что использует ui.js
        isLinkProcessingEnabled = getStorage('isLinkProcessingEnabled', false);
        console.debug(`[_Previewer_Media_] Loaded isLinkProcessingEnabled: ${isLinkProcessingEnabled}`);

        if (isLinkProcessingEnabled) {
            // Чат может ещё не быть в DOM — ждём немного
            setTimeout(() => {
                setupMutationObserver();
                debouncedProcessLinks();
            }, 1000);
        ...
        // Экспорт функций через window.PreviewerMedia
        window.PreviewerMedia = {
            start,
            stop
