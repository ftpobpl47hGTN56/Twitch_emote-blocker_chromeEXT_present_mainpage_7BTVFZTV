...
(function () {
    ...
    async function setEmojiSpamLimit(limit) {
        ...
        if (isNaN(parsedLimit) || parsedLimit < 1) {
            console.warn("[_Emoji_Spam_Protect_] Invalid limit value, using default: 200");
            emojiSpamLimit = 200;
        }
        else {
    ...
    async function initEmojiSpamProtection() {
        ...
        emojiSpamLimit = await getStorage('emojiSpamLimit', 200);
    ...
    async function updateEmojiSpamProtectionUI() {
        ...
        if (emojiSpamLimitInput) {
            const limit = await getStorage('emojiSpamLimit', 200);
