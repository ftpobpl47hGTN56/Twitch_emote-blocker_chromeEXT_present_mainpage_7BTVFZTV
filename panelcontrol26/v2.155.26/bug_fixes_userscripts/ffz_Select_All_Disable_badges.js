...
(function() {
   ...
   // Функция debounce
    ...
    // Функция для проверки и управления кнопкой toggle
    function checkAndToggleBadges() {
        ...
        // Создание кнопки "Select All badges" если её нет
        ...
        if (resetContainer && !resetContainer.querySelector('.ffz-select-all-button')) {
            ...
            selectAllButton.innerHTML = `
                ...
                <span class="tw-button__text">Select All badges</span>
            `;
            selectAllButton.addEventListener('click', () => {
                const checkboxes = badgesSection.querySelectorAll('input.ffz-checkbox__input[type="checkbox"]');
                let allChecked = true;
                checkboxes.forEach(checkbox => {
                    if (!whitelistIds.includes(checkbox.id) && !checkbox.checked) {
                        allChecked = false;
                    }
                });
                ...
                checkboxes.forEach(checkbox => {
                    ...
                    if (checkbox.checked === allChecked) {
                        checkbox.click(); // Симулируем клик для toggle: если allChecked=true, снимем; если false, отметим
                    }
                });
            });
    ...
    const debouncedCheck = debounce(checkAndToggleBadges, 200); // Уменьшил задержку до 200ms для большей отзывчивости
