...
// ===== Версия 2.155.26 от 31.08.2025 clear_twitch_chat_recycler.js
// ===== Версия 2.155.26 от 01.09.2025 clear_twitch_chat_recycler.js
// ===== Версия 2.155.26 от 05.09.2025 clear_twitch_chat_recycler.js
...
(function () {
    ...
    function getStoredLimit() {
        if (window.getStorage) {
            ...
            window.setStorage('chatMessageLimit', 125);
        }
        return 125;
    }
...
(function () {
    ...
    console.log('[_Chat_Recycler_] Скрипт chat_recycler.js загружен ');
       ...
       // ────────────────────── ПЕРСОНАЛЬНЫЙ ЛИМИТ ДЛЯ КАЖДОЙ ВКЛАДКИ ──────────────────────
    // Генерируем уникальный ID вкладки (живёт пока вкладка открыта)
    let tabId = localStorage.getItem('chat_recycler_tab_id');
    if (!tabId) {
        tabId = 'tab_' + Math.random().toString(36).substr(2, 9) + '_' + Date.now();
        localStorage.setItem('chat_recycler_tab_id', tabId);
    }

    // Уникальный ключ для этой вкладки
    const LIMIT_KEY = `chat_recycler_limit_${tabId}`;

    // Читаем лимит, сохранённый именно для этой вкладки (по умолчанию 100)
    let MAX_MESSAGES = parseInt(localStorage.getItem(LIMIT_KEY)) || 65;

    // Функция сохранения нового лимита только для текущей вкладки
    function setTabLimit(newLimit) {
        if (newLimit >= 1) {
            MAX_MESSAGES = newLimit;
            localStorage.setItem(LIMIT_KEY, newLimit);
            console.log(`[_Chat_Recycler_] Лимит для ЭТОЙ вкладки изменён на: ${newLimit}`);
        }
    }

    // При закрытии вкладки — можно почистить (опционально, но аккуратно)
    window.addEventListener('beforeunload', () => {
        // Раскомментируйте строку ниже, если хотите полностью удалять лимит при закрытии вкладки
    ...
    // --- Автоматическое удаление при превышении лимита ---
    async function autoDeleteMessages() {
        let currentLimit = MAX_MESSAGES ? MAX_MESSAGES : MAX_MESSAGES;
        if (typeof MAX_MESSAGES !== 'undefined' && MAX_MESSAGES !== currentLimit) {
            MAX_MESSAGES = currentLimit;
            console.log('[_Chat_Recycler_] Обновлён MAX_MESSAGES:', MAX_MESSAGES);
        }
    ...
    function addMessageLimitSection() {
        ...
        messageLimitButton.addEventListener('click', () => {
            inputContainer.style.display = inputContainer.style.display === 'none' ? 'block' : 'none';
            const input = inputContainer.querySelector('#message-h5j6rj6rmij6rjr5r-limit-input');
            if (input) input.value = MAX_MESSAGES ? MAX_MESSAGES : MAX_MESSAGES;
            console.log('[_Chat_Recycler_] Открыт/закрыт контейнер ввода лимита');
        });
        ...
        inputContainer.addEventListener('input', (e) => {
            if (e.target.id === 'message-h5j6rj6rmij6rjr5r-limit-input') {
                const newLimit = parseInt(e.target.value);
                if (newLimit > 0) {
                    if (typeof setStoredLimit === 'function') setTabLimit(newLimit);
                    MAX_MESSAGES = newLimit;
                    messageLimitButton.querySelector('.hGDXYq').textContent = newLimit;
                    updateTooltip();
                    console.log('[_Chat_Recycler_] Новый лимит сообщений установлен:', newLimit);
